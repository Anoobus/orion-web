...
namespace Orion.Web.Reports
{
    public class PayPeriodReportExcelExportNew
    {
        ...
        {
            var target = GRANDTOTALROW + exemptRows + nonExemptRows + 2;
        ...
        private static void WriteGrandTotalRow(IXLWorksheet excelSheet, int exemptRows, int nonExemptRows)
        {
            ...
            {
                var exemptCell = $"{columns[colIndex]}{INITIALEXEMPTTOTALROW + exemptRows}";
                var nonExemptCell = $"{columns[colIndex]}{INITIALNONEXEMPTTOTALROW + exemptRows + nonExemptRows}";
                excelSheet.Cell(GRANDTOTALROW + exemptRows + nonExemptRows, colIndex + 1).SetFormulaA1($"SUM({exemptCell},{nonExemptCell})");
            }
        }
        ...
        private static void WriteNonExemptEmployees(ReportDTO<PayPeriodReportDTO> rpt, IXLWorksheet excelSheet)
        {
            ...
            var activeRow = INITIALNONEXEMPTTOTALROW + exemptEmployeeRows;
        ...
        private static void WriteExemptEmployees(ReportDTO<PayPeriodReportDTO> rpt, IXLWorksheet excelSheet)
        {
            ...
            {
                var activeRow = INITIALEXEMPTTOTALROW;
                var row = excelSheet.Row(INITIALEXEMPTTOTALROW).InsertRowsAbove(exemptEmployees.Count);
                row.Style.Fill.BackgroundColor = XLColor.NoColor;
                row.Style.Font.Bold = false;
                row.Style.Font.FontColor = XLColor.Black;
